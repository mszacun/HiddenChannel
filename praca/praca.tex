...
\chapter{Projekt protokołu kanału ukrytego}
    ...
    \begin{figure}[h]
            \centering
            \includegraphics[scale=0.6]{hidden_channel_model}
...
\chapter{Badania optymalnych parametrów i jakości kanału ukrytego}
    \section{Zależność opóźnienia wiadomości ukrytych od natężenia napływu wiadomości podstawowych}
        ...
        \subsection{Wnioski}
        % napisz tak bardziej ogólnie, że różnymi parametrami pakietów ukrytych można
        % osiągnąć zmniejszenie i zwiększenie opóźnienia

    \section{Zależność opóźnienia wiadomości podstawowych od natężenia napływu wiadomości ukrytych} \label{OPOZNIENEPODSTAWOWYCHODUKRYTYCH}
        \subsection{Metodologia i cel badania}
        \subsection{Obserwacje}
        \subsection{Wnioski}

    \section{Zależność opóźnienia wiadomości podstawowych od natężenia napływu wiadomości podstawowych} \label{BADANIEDLUGOSCISEGMENTUDANYCHUKRYTYCH}
        \subsection{Metodologia i cel badania}
        \subsection{Obserwacje}
        \subsection{Wnioski}
...
\chapter{Analiza wykrywalności kanału ukrytego}
    ...
    \section{Metody ataku na kanał ukryty}
       ...
       swój cel. Należy zauważyć, że dzięki braku obsługi błędów w kanale ukrytym
       ...
       jako \( 00100 01100 \), przykład ten pokazuje, że aby odczytać zawartość wiadomości
