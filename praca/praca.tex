...
\chapter{Charakterystyka możliwości realizacji kanałów ukrytych w sieciach komputerowych}
    ...
    \section{Podejścia hybrydowe}
        ...
        uszkodzony, odrzuca go, jednak odbiorca wiadomości wiedząc, że to właśnie
...
\chapter{Projekt protokołu kanału ukrytego}
    ...
    interpretację długości pakietów jako fragmentów wiadomości ukrytej.
    Kanał ukryty przeznaczony jest do komunikacji typu punkt-punkt pomiędzy odbiorcą a nadawcą.
    Jako protokół usługowy dla protokołu kanału ukrytego wybrany został protokół UDP. Kanał ukryty
    może być wykorzystywany zarówno do komunikacji wewnątrz sieci lokalnej jak i
    ...
    Z uwagi na to, że protokołem usługowym dla kanału ukrytego jest UDP, naturalnym jest
    ...
    Model kanału ukrytego został przedstawiony na rysunku poniżej. W modelu tym zakłada się
    istnienie źródła danych podstawowych, którym tak jak wcześniej opisano, jest
    ...
    Źródło wiadomości ukrytych również charakteryzowane poprzez natężenie
    ...
    Ostatnim elementem środowiska działania kanału ukrytego, jest opis kanału usługowego.
    Charakteryzowany jest on poprzez podanie jego przepływności w bitach na sekundę.
    Przepływność ta może ograniczać ilość danych możliwych do przesłania kanałem
    w jednostce czasu i w rezultacie wpływać na opóźnienie zarówno wiadomości
